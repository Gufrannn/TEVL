"""
run evaluation of Video Retrival
"""
import argparse
import os
...
def main(opts):
    ...
    q_txt_db = TvrQueryTokLmdb(opts.query_txt_db, -1)
    if opts.task != "tvr_video_only":
    ...
    model = TEVLForVr.from_pretrained(
        model_config,
...
def validate_full_vr(model, val_loader, split, opts, model_opts):
    ...
    st = time()
    val_log = {}
    has_gt_target = True  # TVR test set has annotations
    try:
    ...
    for batch in tqdm(val_loader, desc="Computing q2vScores"):
        ...
        for qid in qids:
            # fix tvr query data to have tvr format
            gt = query_data[qid]
            gt["desc_id"] = qid
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", choices=["tvr_video_sub",
                                           "tvr_video_only"],
                        default="tvr_video_sub", type=str,
